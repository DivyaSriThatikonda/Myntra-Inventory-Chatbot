BRAND_NAMES = [
    ...
    "9teenagain", "wyfees", "ms.lingies", "masculino latino", "actimaxx", "sakhi ja folytassuk ipur", "silai bunai", "prettybold",
    ...
    "woods", "varkha fashion", "anekaant", "copperline", "evoq", "kur creando ti's by menka", "siah", "ratan creation",
    "pyjama party", "cotclo", "h&m", "puroh", "zotw", "lux cozi for her", "sztori marvel", "aarke ritu kumar",
    ...
    "ginger by lifestyle quartile", "shyaway", "cayman", "creeva", "american crew plus", "zoeyams", "sf jeans by pantaloons",
...
SAMPLE_QUESTIONS = {
    ...
    "show me tshirts from campus sutra with low stock": "SELECT brandname, individual_category, size, current_stock, reorder_level, discountpriceinrs FROM sales_and_stock_info WHERE brandname = 'campus sutra' AND individual_category = 'tshirts' AND current_stock <= reorder_level LIMIT 10",
    "what is the total revenue for indian wear": "SELECT SUM(revenueinrs) FROM sales_and_stock_info WHERE category = 'indian wear'",
    "list the top-rated dresses": "SELECT brandname, individual_category, ratings, reviews FROM myntra_fashion_clothing WHERE individual_category = 'dresses' ORDER BY ratings::FLOAT DESC LIMIT 1",
    ...
    "what is the average rating for tops": "SELECT AVG(ratings::FLOAT) AS avg_rating FROM myntra_fashion_clothing WHERE individual_category = 'tops'",
    "how many jeans were sold in size m in the next 30 days": "SELECT SUM(quantity_sold) FROM sales_and_stock_info WHERE individual_category = 'jeans' AND size = 'm' AND quantity_sold > 0 AND predicted_stockout_date <= CURRENT_DATE + INTERVAL '30 days'",
    ...
    "cheapest tshirts for men": "SELECT brandname, description, discountpriceinrs, current_stock FROM sales_and_stock_info WHERE individual_category = 'tshirts' AND category_by_gender = 'men' ORDER BY discountpriceinrs ASC LIMIT 5",
    "highest rated jeans with more than 100 reviews": "SELECT brandname, individual_category, ratings, reviews FROM myntra_fashion_clothing WHERE individual_category = 'jeans' AND reviews > 100 ORDER BY ratings::FLOAT DESC LIMIT 5",
    "restock priority for high turnover items": "SELECT brandname, individual_category, size, current_stock, reorder_level, predicted_restock_quantity, predicted_restock_date FROM sales_and_stock_info WHERE turnover_flag = 'High' AND current_stock <= reorder_level ORDER BY current_stock::FLOAT / NULLIF(reorder_level, 0) ASC"
}

PROMPT_TEMPLATE = """
You are an expert SQL query generator for a retail inventory database with two tables: 'sales_and_stock_info' and 'myntra_fashion_clothing'. Given a natural language query, generate a precise SQL query for PostgreSQL.

**Table: sales_and_stock_info**
Columns: product_id (BIGINT, PRIMARY KEY), brandname (VARCHAR(100)), category (VARCHAR(50)), individual_category (VARCHAR(50)), category_by_gender (VARCHAR(20)), description (TEXT), discountpriceinrs (FLOAT), current_stock (INTEGER), reorder_level (INTEGER), quantity_sold (INTEGER), revenueinrs (FLOAT), turnover_flag (VARCHAR(20)), predicted_restock_quantity (INTEGER), predicted_stockout_date (DATE), predicted_restock_date (DATE), size (VARCHAR(20))
Sample brands: {brand_samples}
Sample categories: {category_samples}
Sample individual categories: {individual_category_samples}

**Table: myntra_fashion_clothing**
Columns: url (VARCHAR(255)), product_id (BIGINT, PRIMARY KEY), brandname (VARCHAR(100)), category (VARCHAR(50)), individual_category (VARCHAR(50)), category_by_gender (VARCHAR(20)), description (TEXT), discountpriceinrs (FLOAT), originalpriceinrs (DOUBLE PRECISION), discountoffer (VARCHAR(50)), ratings (DECIMAL(3,1)), reviews (INTEGER), standardsize (VARCHAR(100)), discountpercent (INTEGER)
Sample brands: {brand_samples}
Sample categories: {category_samples}
Sample individual categories: {individual_category_samples}
...
- For ratings, reviews, or discounts, use 'myntra_fashion_clothing'.
...
- For ratings, use myntra_fashion_clothing.ratings (DECIMAL(3,1)); cast to FLOAT for sorting (e.g., ratings::FLOAT).
- For time-based queries, use predicted_stockout_date or predicted_restock_date (e.g., WHERE predicted_stockout_date <= CURRENT_DATE + INTERVAL '30 days').
...
- For description-based queries (e.g., 'blue jeans'), use description ILIKE '%blue%'.
- Use PostgreSQL syntax: LIMIT instead of TOP, CURRENT_DATE instead of GETDATE(), INTERVAL for date arithmetic, || for string concatenation.
- Ensure column names are lowercase (e.g., brandname, not BrandName).
